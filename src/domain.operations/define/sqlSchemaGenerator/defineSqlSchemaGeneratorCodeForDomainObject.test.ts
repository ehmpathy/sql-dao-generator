...
describe('defineSqlSchemaGeneratorCodeForDomainObject', () => {
  describe('imports', () => {
    ...
    it('should create an import for each directly nested and uuid inferred reference, sorted', () => {
      ...
      expect(importLines[1]).toContain("from './badge.ts'"); // b, is first in alphabetical order of subsequent imports
      expect(importLines[2]).toContain("from './engineer.ts'"); // e, is next
      expect(importLines[3]).toContain("from './locomotive.ts'"); // l, is after that
      expect(importLines[4]).toContain("from './station.ts'"); // s, is last
