...
describe('defineQueryFunctionInputExpressionForDomainObjectProperty', () => {
  describe('for upsert', () => {
    ...
    it('should define the input expression correctly for a solo, nullable, DIRECT_BY_DECLARATION reference', () => {
      ...
      expect(expression).toEqual(
        `carriageUuid: carriageCargo.carriageRef === null ? null : isRefByPrimary({ of: Carriage })(carriageCargo.carriageRef) ? carriageCargo.carriageRef.uuid : (await carriageDao.findByRef({ ref: carriageCargo.carriageRef }, context).expect('isPresent')).uuid`,
      );
    });
