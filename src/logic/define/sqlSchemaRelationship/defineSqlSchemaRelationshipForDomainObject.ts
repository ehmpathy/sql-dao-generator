...
export const defineSqlSchemaRelationshipForDomainObject = ({
  ...
  const databaseGeneratedSqlSchemaProperties = defineDatabaseGeneratedSqlSchemaPropertiesForDomainObject({
    domainObject,
  });
  const propertiesRelationship = [
    ...
    ...databaseGeneratedSqlSchemaProperties.map((sqlSchemaProperty) => {
      ...
      if (domainObjectProperty) {
        const expectedType = (() => {
          if (sqlSchemaProperty.name === 'id') return DomainObjectPropertyType.NUMBER;
          if (sqlSchemaProperty.name === 'uuid') return DomainObjectPropertyType.STRING;
    ...
    ...Object.values(domainObject.properties)
      .filter(
        (domainObjectProperty) =>
          !databaseGeneratedSqlSchemaProperties.map(({ name }) => name).includes(snakeCase(domainObjectProperty.name)), // filter out the db generated values, because we already got them above
      )
  ...
  const sqlSchemaUniqueKeys = (() => {
    ...
    const domainObjectPropertyNamesInUniqueKey =
      domainObject.extends === DomainObjectVariant.DOMAIN_VALUE_OBJECT
        ? propertiesRelationship // if its a value object, the schema is unique on all properties of the object (excluding autogenerated ones like id and uuid)
            .filter((propertyRelationship) => !propertyRelationship.sqlSchema.isDatabaseGenerated)
