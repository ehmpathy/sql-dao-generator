  ...
  describe('reference: direct_by_nesting', () => {
    ...
    it('should define the select expression correctly for a solo DIRECT_BY_NESTING reference with its own solo DIRECT_BY_NESTING reference', () => {
      const expression = defineQuerySelectExpressionForSqlSchemaProperty({
        ...
        allSqlSchemaRelationships: [
          new SqlSchemaToDomainObjectRelationship({
            name: { domainObject: 'InvoiceLineItem', sqlSchema: 'invoice_line_item' },
            properties: [
              {
                domainObject: {
                  name: 'id',
          ...
          new SqlSchemaToDomainObjectRelationship({
            ...
            properties: [
              {
                domainObject: { name: 'amount', type: DomainObjectPropertyType.NUMBER },
                sqlSchema: {
                  name: 'amount',
                  isArray: false,
                  isNullable: false,
              ...
              {
                domainObject: { name: 'currency', type: DomainObjectPropertyType.ENUM, of: ['USD'] },
                sqlSchema: {
                  name: 'currency',
                  isArray: false,
                  isNullable: false,
      ...
      expect(expression).toContain('SELECT json_build_object'); // should grab a json object
      expect(expression).toContain('FROM invoice_line_item WHERE invoice_line_item.id = invoice.line_item_id'); // from the right table
      expect(expression).toContain('FROM price WHERE price.id = invoice_line_item.price'); // from the right table
    ...
    it('should define the select expression correctly for an array DIRECT_BY_NESTING reference with its own solo DIRECT_BY_NESTING reference', () => {
      ...
      expect(expression).toContain('ON invoice_line_item.id = invoice_line_item_ref.id');
      expect(expression).toContain('FROM price WHERE price.id = invoice_line_item.price'); // from the right table
      expect(expression).toContain('AS line_item'); // with the correct output name
      expect(expression).toMatchSnapshot();
    });
