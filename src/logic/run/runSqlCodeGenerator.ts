...
export const runSqlCodeGenerator = async ({
  config,
}: {
  ...
  const result = await shell.exec(
    `npx sql-code-generator generate --config=${config.generates.code.config.path}`,
    {
      silent: true,
    },
  );
  ...
  const successMessage = `  ${chalk.green('✔')} ${chalk.green(
    chalk.bold('[RAN]'),
  )} ${chalk.bold('sql-code-generator 🏃')}`;
  console.log(successMessage); // tslint:disable-line no-console

  // generate the sub messages
  const successMessage2 = `    ${chalk.green('✔')} ${chalk.green(
    chalk.bold('[GENERATED]'),
  )} ${chalk.bold(config.generates.code.config.content.generates.types)}`;
  console.log(successMessage2); // tslint:disable-line: no-console
  const successMessage3 = `    ${chalk.green('✔')} ${chalk.green(
    chalk.bold('[GENERATED]'),
  )} ${chalk.bold(
