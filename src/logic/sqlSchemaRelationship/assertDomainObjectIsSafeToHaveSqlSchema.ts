...
export const assertDomainObjectIsSafeToHaveSqlSchema = ({ domainObject }: { domainObject: DomainObjectMetadata }) => {
  ...
  if (domainObject.extends === DomainObjectVariant.DOMAIN_ENTITY) {
    if (!domainObject.decorations.unique?.length)
      throw new Error(
        `domain entities must have at least one 'unique' property defined in order for a schema to be generated. '${domainObject.name}' does not satisfy this requirement.
    ...
    if (!Array.isArray(domainObject.decorations.updatable))
      throw new Error(`domain entities must have their 'updatable' properties defined in order for a schema to be generated. '${domainObject.name}' does not satisfy this requirement.
  ...
  if (domainObject.extends === DomainObjectVariant.DOMAIN_VALUE_OBJECT) {
    if (domainObject.decorations.unique)
      throw new Error(
        `domain value objects must _not_ have their 'unique' properties specified. value objects are unique on all of their properties by definition. '${domainObject.name}' does not satisfy this requirement.`,
      );
    if (domainObject.decorations.updatable)
      throw new Error(
        `domain value objects must _not_ have any 'updatable' properties specified. value objects do not have any updatable properties by definition. '${domainObject.name}' does not satisfy this requirement.`,
      );
  }
  ...
  if (domainObject.extends === DomainObjectVariant.DOMAIN_EVENT) {
    if (!domainObject.decorations.unique?.length)
      throw new Error(
        `domain events must have at least one 'unique' property defined in order for a schema to be generated. '${domainObject.name}' does not satisfy this requirement.`,
      );
  }
};
